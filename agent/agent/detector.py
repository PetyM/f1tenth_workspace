...
class Detector(rclpy.node.Node):
    ...
    def timer_cb(self):
        collision, _ = cv2.rotatedRectangleIntersection(((self.ego_state[0], self.ego_state[1]), (0.75 * self.vehicle_width, 0.75 * self.vehicle_length), math.degrees(self.ego_state[2])),
                                                        ((self.opp_state[0], self.opp_state[1]), (0.75 * self.vehicle_width, 0.75 * self.vehicle_length), math.degrees(self.opp_state[2])))
        if collision != 0:
