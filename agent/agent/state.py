...
def nearest_point_on_trajectory(point: np.ndarray, trajectory: np.ndarray):
    ...
    dots = np.empty((trajectory.shape[0] - 1,), dtype=np.float64)
    ...
    dists = np.empty((projections.shape[0],), dtype=np.float64)
...
def first_point_on_trajectory_intersecting_circle(
    ...
    for i in range(start_i, trajectory.shape[0] - 1):
        ...
        a = np.dot(V, V)
        b = np.float64(2.0) * np.dot(
            V, start - point
        )  # NOTE: specify type or numba complains
        c = (
            np.dot(start, start)
            + np.dot(point, point)
            - np.float64(2.0) * np.dot(start, point)
            - radius * radius
        )
